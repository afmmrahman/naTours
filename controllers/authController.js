...
exports.login = catchAsync(async (req, res, next) => {
  ...
  // 1) If the email and password exist
  if (!email || !password) {
...
exports.protect = catchAsync(async (req, res, next) => {
  //1) Get token and check it if it is existing
  let token;
  if (
...
exports.forgetPassword = catchAsync(async (req, res, next) => {
  ...
  if (!user) {
    return next(new AppError('There is no user with this emain address.', 404));
  }
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  // 2) If tokenhas not expired and there is a user, set the new password
  if (!user) {
  ...
  user.passwordResetExpires = undefined;
  await user.save();
  // 3) Update changedPasswordAt propertyfor the users
